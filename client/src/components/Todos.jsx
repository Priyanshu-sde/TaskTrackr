...
const Todos = ({ token }) => {
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      const response = await axios.get('https://task-trackr-server-seven.vercel.app/todos', {
  ...
  const updateTodo = async (id) => {
    const response = await axios.put(`https://task-trackr-server-seven.vercel.app/todos/${id}`, { title: newTitle }, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  const toggleDone = async (id, done) => {
    await axios.patch(`https://task-trackr-server-seven.vercel.app/${id}/done`, { done: !done }, {
      headers: { Authorization: `Bearer ${token}` }
    });
  ...
  const deleteTodo = async (id) => {
    await axios.delete(`https://task-trackr-server-seven.vercel.app/todos/${id}`, {
      headers: { Authorization: `Bearer ${token}` }
    });
